    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div x-data="{ 
                ...
                matchesSearch(productName, sellerName, categoryName, status) {
                    const query = this.search[status].toLowerCase().trim();
                    if (!query) return true;
                    const nameMatch = productName.toLowerCase().includes(query);
                    const sellerMatch = sellerName.toLowerCase().includes(query);
                ...
                sortRows(status) {
                    setTimeout(() => {
                        ...
                        const visibleRows = allRows.filter(row => {
                            ...
                            const sellerNameCell = row.querySelector('td:nth-child(2)');
                            const categoryCell = row.querySelector('td:nth-child(3)');
                            if (!productNameCell || !sellerNameCell || !categoryCell) return false;
                            const productName = productNameCell.textContent.trim();
                            const sellerName = sellerNameCell.textContent.trim();
                            const categoryName = categoryCell.textContent.trim();
                            return this.matchesSearch(productName, sellerName, categoryName, status);
                        });
                    ...
                    <div class="mb-4 flex items-center justify-between gap-4">
                        ...
                        <div class="flex items-center gap-2">
                            ...
                            >
                                ...
                                <option value="a-z">A → Z (Item Name)</option>
                                <option value="z-a">Z → A (Item Name)</option>
                            </select>
                        </div>
                    </div>
                    <div class="overflow-x-auto">
                        ...
                        <div 
                            x-show="search.pending.trim() !== '' && Array.from($el.previousElementSibling.querySelectorAll('tbody tr')).filter(row => {
                                ...
                                const sellerName = sellerNameCell.textContent.trim();
                                const categoryName = categoryCell.textContent.trim();
                                return matchesSearch(productName, sellerName, categoryName, 'pending') && row.offsetParent !== null;
                            }).length === 0"
                            x-cloak
                    ...
                    <div class="overflow-x-auto">
                        ...
                        <div 
                            x-show="search.approved.trim() !== '' && Array.from($el.previousElementSibling.querySelectorAll('tbody tr')).filter(row => {
                                ...
                                const sellerName = sellerNameCell.textContent.trim();
                                const categoryName = categoryCell.textContent.trim();
                                return matchesSearch(productName, sellerName, categoryName, 'approved') && row.offsetParent !== null;
                            }).length === 0"
                            x-cloak
                            class="text-center py-10 text-gray-500 dark:text-gray-400 mt-4"
                        >
                    ...
                    <div class="overflow-x-auto">
                        ...
                        <div 
                            x-show="search.rejected.trim() !== '' && Array.from($el.previousElementSibling.querySelectorAll('tbody tr')).filter(row => {
                                ...
                                const sellerName = sellerNameCell.textContent.trim();
                                const categoryName = categoryCell.textContent.trim();
                                return matchesSearch(productName, sellerName, categoryName, 'rejected') && row.offsetParent !== null;
                            }).length === 0"
                            x-cloak
                            class="text-center py-10 text-gray-500 dark:text-gray-400 mt-4"
                        >
