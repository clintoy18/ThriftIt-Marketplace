            ...
            <article class="bg-white/80 dark:bg-gray-800/80 backdrop-blur-xl rounded-3xl shadow-xl border border-gray-200/50 dark:border-gray-700/50 overflow-hidden transform transition-all duration-500 hover:shadow-2xl hover:-translate-y-1">
                ...
                <div class="p-6" x-data="{ expanded: false }">
                    ...
                    <div class="mb-6">
                        ...
                        {{-- Only show button if text is longer than 200 chars --}}
                        @if(Str::length($post->content) > 200)
                            <button @click="expanded = !expanded" 
                                    class="mt-2 text-sm font-bold text-emerald-600 dark:text-emerald-400 hover:text-emerald-700 dark:hover:text-emerald-300 focus:outline-none flex items-center gap-1 transition-colors">
                    ...
                    @if ($post->image)
                        <div class="mb-6 rounded-2xl overflow-hidden border border-gray-200 dark:border-gray-700 shadow-lg">
                            <img src="{{ Storage::disk('s3')->url($post->image) }}" alt="Post Image" class="w-full object-cover h-96 hover:scale-105 transition-transform duration-500">
                        </div>
                    @endif

                    @if ($post->video_link)
                        <a href="{{ $post->video_link }}" target="_blank" class="inline-flex items-center gap-3 text-emerald-600 dark:text-emerald-400 font-semibold hover:underline">
