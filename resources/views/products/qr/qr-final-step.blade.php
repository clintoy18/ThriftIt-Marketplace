...
<x-app-layout>
    ...
    <div class="py-8">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            ...
            @else
                <div
                    class="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-2xl p-4 mb-8">
                    <div class="flex items-center gap-4">
                        <div class="flex-shrink-0">
                            <div
                                class="w-10 h-10 bg-blue-100 dark:bg-blue-900/40 rounded-full flex items-center justify-center">
                        ...
                        <div class="flex-1">
                            ...
                            <p class="text-xs text-blue-700 dark:text-blue-300 mt-0.5">
                                You are publishing as a Standard Seller (No QR).
                            </p>
                        </div>
                    </div>
                </div>
            ...
            <div class="bg-white dark:bg-gray-800 shadow-2xl sm:rounded-3xl overflow-hidden border-0">
                ...
                <div class="p-8">
                    ...
                    <div
                        ...
                        @if (!empty($images))
                            <div class="grid grid-cols-2 sm:grid-cols-4 gap-4">
                                @foreach ($images as $image)
                                    <div
                                        class="relative aspect-square rounded-lg overflow-hidden border border-gray-200 dark:border-gray-700 shadow-sm group">

                                        {{-- FIX: Use S3 disk URL instead of local asset --}}
                                        <img src="{{ Storage::disk('s3')->url($image) }}" alt="Product preview"
                        ...
                        @else
                            <p class="text-sm text-red-500 italic">No images found. Please go back and upload photos.
                            </p>
                        @endif
                    </div>
                        ...
                        <div class="grid md:grid-cols-2 gap-4">
                            <div class="space-y-3">
                                <div
                                    ...
                                    <span
                                        class="text-gray-800 dark:text-white font-semibold">{{ $step1['name'] ?? 'N/A' }}</span>
                                </div>
                                <div
                                    class="flex justify-between items-center py-2 border-b border-[#F1E9D2] dark:border-gray-600">
                                    <span class="text-sm text-gray-600 dark:text-gray-400 font-medium">Price</span>
                                    <span
                                        class="text-green-600 dark:text-green-400 font-bold">₱{{ number_format($step1['price'] ?? 0, 2) }}</span>
                                </div>
                                <div class="flex justify-between items-center py-2">
                                    <span class="text-sm text-gray-600 dark:text-gray-400 font-medium">Category</span>
                                    <span
                                        class="text-gray-800 dark:text-white font-semibold">{{ $category->name ?? '-' }}</span>
                                </div>
                            </div>
                            <div class="space-y-3">
                                <div
                                    ...
                                    <span
                                        class="text-gray-800 dark:text-white font-semibold">{{ $segment->name ?? '-' }}</span>
                                </div>
                                <div class="flex justify-between items-center py-2">
                                    <span class="text-sm text-gray-600 dark:text-gray-400 font-medium">Barangay</span>
                                    <span
                                        class="text-gray-800 dark:text-white font-semibold">{{ $barangay->name ?? '-' }}</span>
                                </div>
                            </div>
                        </div>
                    </div>
                        ...
                        @if ($qr)
                            <div class="flex flex-col items-center text-center">
                                <div
                                    ...
                                    <img src="{{ Storage::disk('public')->url($qr) }}" alt="QR Code"
                    ...
                    <form action="{{ route('sell-item.finalize') }}" method="POST">
                        @csrf
                        <div
                            class="flex flex-col sm:flex-row gap-4 pt-8 border-t border-gray-100 dark:border-gray-700">
                            ...
                            @else
                                <a href="{{ route('products.create') }}"
                                    ...
                                    <div class="text-left flex-1">
                                        <div class="text-sm font-semibold text-gray-500 dark:text-gray-400">Edit
                                            Details</div>
            ...
            <div class="mt-8 text-center">
                <div
                    class="inline-flex items-center gap-3 bg-gray-50 dark:bg-gray-800 rounded-2xl px-5 py-3 shadow-md border border-gray-200 dark:border-gray-700">
                    <svg class="w-5 h-5 text-gray-500 dark:text-gray-300" fill="none" stroke="currentColor"
                        viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            d="M8 10h.01M12 10h.01M16 10h.01M9 16h6m2 4H7a2 2 0 01-2-2V6a2 2 0 012-2h5l2 2h6a2 2 0 012 2v10a2 2 0 01-2 2z" />
                    </svg>
