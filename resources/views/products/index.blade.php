<x-app-layout>
    ...
    <div class="py-6 bg-white dark:bg-gray-900">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            {{-- LOGIC TO SEPARATE AVAILABLE AND SOLD --}}
            @php
                // Filter the passed $approved collection. 
                // Adjust 'sold' check based on your DB (e.g. $p->is_sold or $p->status === 'sold')
                $soldItems = $approved->filter(fn($p) => $p->status === 'sold' || $p->is_sold);
                $availableItems = $approved->diff($soldItems);
            @endphp

            <div class="flex items-center justify-between mb-8">
                <h2 class="text-2xl font-extrabold tracking-tight text-gray-900 dark:text-gray-100">Inventory Management</h2>
                <a href="{{ route('products.create') }}"
            ...
            @if ($rejected->count() > 0)
                <div class="mb-10">
                    ...
                    <p class="mt-2 text-sm text-red-500 italic pl-1">* Click items to see rejection reasons and edit.</p>
                </div>
            ...
            {{-- SECTION 3: AVAILABLE ITEMS --}}
            <div class="mb-10">
                <div class="flex items-center gap-2 mb-4 px-1">
                    <div class="w-1.5 h-6 bg-green-500 rounded-full"></div>
                    <h3 class="text-xl font-bold text-gray-800 dark:text-white">Available Items</h3>
                    <span class="bg-green-100 text-green-800 text-xs font-bold px-2.5 py-0.5 rounded-full">{{ $availableItems->count() }}</span>
                </div>

                @if ($availableItems->count() > 0)
                    <div class="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2 sm:gap-4 md:gap-6">
                        @foreach ($availableItems as $product)
                            <x-item-card :product="$product" />
                        @endforeach
                    </div>
                @else
                    <div class="bg-gray-50 dark:bg-gray-800 rounded-xl p-8 text-center border border-dashed border-gray-200 dark:border-gray-700">
                        ...
                        <h3 class="text-lg font-medium text-gray-900 dark:text-white">No available listings</h3>
                        <p class="mt-1 text-sm text-gray-500 dark:text-gray-400 mb-6">Start selling your pre-loved fashion today.</p>
                        <a href="{{ route('products.create') }}" class="inline-flex items-center px-4 py-2 bg-[#B59F84] border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-[#a08e77] transition">
                            List Item
                        </a>
                    </div>
                @endif
            ...
            {{-- SECTION 4: SOLD ITEMS --}}
            <div class="mb-10">
                <div class="flex items-center gap-2 mb-4 px-1">
                    <div class="w-1.5 h-6 bg-gray-500 rounded-full"></div>
                    <h3 class="text-xl font-bold text-gray-800 dark:text-white">Sold Items</h3>
                    <span class="bg-gray-100 text-gray-800 text-xs font-bold px-2.5 py-0.5 rounded-full">{{ $soldItems->count() }}</span>
                </div>

                @if ($soldItems->count() > 0)
                    <div class="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2 sm:gap-4 md:gap-6 opacity-80">
                ...
                @else
                    <div class="bg-gray-50 dark:bg-gray-800 rounded-xl p-6 text-center border border-dashed border-gray-200 dark:border-gray-700">
                        <p class="text-gray-500 text-sm">No items sold yet. Keep listing!</p>
                    </div>
                @endif
            </div>
