<x-app-layout>
    ...
    <div class="py-12">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-[#F4F2ED] dark:bg-gray-900 shadow-xl rounded-2xl border border-gray-200 dark:border-gray-700 p-8 sm:p-10">
                
                <form action="{{ route('appointments.store') }}" method="POST" class="space-y-8" enctype="multipart/form-data">
                    @csrf

                    @if ($upcycler)
                        <div class="bg-white dark:bg-gray-800 rounded-xl p-6 border border-gray-200 dark:border-gray-700 border-l-4 border-l-[#B59F84] shadow-sm flex items-start justify-between">
                            <div>
                    ...
                    <div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div class="space-y-2">
                                <label for="apptype" class="block text-sm font-semibold text-gray-700 dark:text-gray-300">Service Type</label>
                                ...
                                @error('apptype') <p class="text-sm text-red-600 font-medium">{{ $message }}</p> @enderror
                            </div>

                            <div class="space-y-2">
                                <label for="appdate" class="block text-sm font-semibold text-gray-700 dark:text-gray-300">Preferred Date</label>
                                <input type="date" id="appdate" name="appdate" value="{{ old('appdate') }}" min="{{ now()->format('Y-m-d') }}" required
                                    class="w-full px-4 py-3 rounded-xl border-gray-300 dark:border-gray-600 bg-gray-50 dark:bg-gray-800 focus:bg-white dark:focus:bg-gray-700 focus:ring-2 focus:ring-[#B59F84] focus:border-[#B59F84] transition shadow-sm">
                            ...
                            <div class="space-y-2 md:col-span-2">
                                <label for="appdetails" class="block text-sm font-semibold text-gray-700 dark:text-gray-300">
                                ...
                                @error('appdetails') <p class="text-sm text-red-600 font-medium">{{ $message }}</p> @enderror
                            </div>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <div class="bg-blue-50 dark:bg-blue-900/20 border border-blue-100 dark:border-blue-800 rounded-xl p-4 mb-5">
                            <div class="flex gap-3">
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-blue-600 dark:text-blue-400 shrink-0 mt-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                                </svg>
                                <div>
                                    <h4 class="text-sm font-bold text-blue-800 dark:text-blue-300 mb-1">Photo Guidelines</h4>
                                    <p class="text-xs text-blue-700 dark:text-blue-300 leading-relaxed">
                                        Upload <strong>Before</strong> photos, <strong>Inspiration</strong> references, and <strong>Close-ups</strong> of details. Limit 8 photos (Max 5MB each).
                                    </p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="relative">
                            <label for="appointmentImages" id="appointmentDropZone"
                                ...
                                <div id="appointmentPreviews" class="grid grid-cols-2 sm:grid-cols-4 gap-4 w-full mb-2 empty:hidden"></div>

                                <div id="appointmentAddMoreText" class="text-center py-2 hidden">
                                    <span class="text-sm text-[#B59F84] font-bold bg-[#B59F84]/10 px-3 py-1 rounded-full">+ Add more photos</span>
                                </div>

                                <div id="appointmentDropZoneContent" class="flex flex-col items-center justify-center text-center gap-3 py-6">
                                    <div class="w-16 h-16 rounded-full bg-gray-100 dark:bg-gray-700 flex items-center justify-center text-gray-400 group-hover:text-[#B59F84] group-hover:scale-110 transition duration-300">
                        ...
                        <div class="mt-2 space-y-1">
                            ...
                            <p id="appointmentReachLimitError" class="text-sm text-red-600 font-bold hidden">Limit reached (Max 8 photos).</p>
                        </div>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function renderPreviews(files) {
            ...
            files.forEach((file, index) => {
                ...
                wrapper.className = 'relative group h-24 w-full rounded-lg overflow-hidden shadow-sm border border-gray-200';
                
                const img = document.createElement('img');
                img.className = 'w-full h-full object-cover';
                
                const badge = document.createElement('div');
                badge.className = 'absolute top-1 left-1 bg-black/60 text-white text-[10px] px-1.5 rounded-sm backdrop-blur-sm';
        ...
        dropZone.addEventListener('dragleave', () => dropZone.classList.remove('border-[#B59F84]', 'bg-gray-50'));
